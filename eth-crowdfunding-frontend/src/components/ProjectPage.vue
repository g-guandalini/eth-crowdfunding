<template>
  ...
  async function loadProjectDetails(id: number) {
    ...
    try {
      ...
      try {
        const signer = await provider.getSigner();
        currentConnectedAddress = await signer.getAddress();
        connectedWalletAddress.value = currentConnectedAddress;
      } catch (e) {
        console.warn("Nenhuma carteira conectada ou permissão negada para obter endereço.", e);
        // connectedWalletAddress já é null por padrão aqui
      }
      ...
      if (currentConnectedAddress) {
        try {
          const donation = await contract.getDonation(id, currentConnectedAddress);
      ...
      project.value = {
        ...
        withdrawn: p.withdrawn, // Adicionado
      ...
      project.value = null;
      if (error.code === "CALL_EXCEPTION" && (error.data === "0x" || error.data?.message?.includes("Invalid project ID"))) {
        toast.error("Projeto não encontrado ou ID inválido na blockchain.");
      } else {
        toast.error("Ocorreu um erro ao carregar os detalhes do projeto. Verifique a conexão com a blockchain.");
      }
    } finally {
      loading.value = false;
    }
  ...
  const canClaimRefund = computed(() => {
    ...
    return (
      ...
      hasDonation // O usuário atual tem uma doação para reivindicar
    );
  });
